 ...
 * `allOrNothing` - flag to specify whether the order should be fulfilled completely or not.
   * *"1"* - The order must be fulfilled completely and entire amount should be traded. If it is not possible, it results in error.
   * *"0"* - The order can be partially fulfilled and the rest of the amount will be kept in orderbook so that it can be traded later. This is the default behaviour if the parameter is not send.
